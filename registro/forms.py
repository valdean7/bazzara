    ...
    def clean(self):
        ...
        if cleaned:
            ...
            if cleaned.get('username') and len(cleaned.get('username')) < 3:
                validation_error_msg['username'] = 'Nome de usuário precisa ter 3 ou mais caracteres.'
            ...
            if cleaned.get('password') and len(cleaned.get('password')) < 8:
                validation_error_msg['password'] = 'Senha deve ter 8 ou mais caracteres.'

            if cleaned.get('password2') and len(cleaned.get('password2')) < 8:
                validation_error_msg['password2'] = 'Senha deve ter 8 ou mais caracteres.'

            if cleaned.get('password') != cleaned.get('password2'):
                validation_error_msg['password2'] = 'Senha não coincide com a anterior.'
